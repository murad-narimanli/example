...
const Tools = (props) => {
  ...
  const search = (values) => {
    agros.get("statistics/tools", { params: formatParams(values) }).then((res) => {
      setData(
  ...
  return (
    <Form form={form} onFinish={search} layout="vertical" className="mt-10">
      <Row gutter={16}>
        ...
        <Col span={6}>
          <Form.Item name="toolId" label="Alət">
            <Select
              className="w-100"
              allowClear
              onChange={(e) => handleKeyChange(e, "toolId")}
            >
...
export default connect(mapStateToProps, { getOptions })(Tools);

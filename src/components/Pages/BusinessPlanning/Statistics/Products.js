...
const Products = (props) => {
  ...
  const search = (values) => {
    agros.get("statistics/crops", { params: formatParams(values) }).then((res) => {
      setData(
  ...
  return (
    <Form form={form} onFinish={search} layout="vertical" className="mt-10">
      <Row gutter={16}>
        ...
        <Col span={6}>
          <Form.Item name="cropCategoryId" label="Məhsul kateqoriyası">
            <Select
              className="w-100"
              onChange={(e) => handleKeyChange(e, "cropCategoryId")}
              allowClear
            >
        ...
        <Col span={6}>
          <Form.Item name="cropId" label="Məhsul">
            <Select
              className="w-100"
              onChange={(e) => handleKeyChange(e, "cropId")}
              allowClear
            >
              {options.crops
                .filter((f) => f.categoryId === filters.cropCategoryId)
                .map((f, index) => {
                  return (
                    <Option key={index} value={f.id}>
                      {f.name}
        ...
        <Col span={6}>
          <Form.Item name="cropSortId" label="Məhsul sortu">
            <Select
              className="w-100"
              onChange={(e) => handleKeyChange(e, "cropSortId")}
              allowClear
            >
              {options.cropSorts
                .filter((f) => f.categoryId === filters.cropId)
                .map((f, index) => {
                  return (
                    <Option key={index} value={f.id}>
                      {f.name}
...
export default connect(mapStateToProps, { getOptions })(Products);
